...
async def _proxy_responses_impl(request: Request):
    ...
    # Extract system content from messages[0] (anchors now in messages, not top-level)
    sys_content = str(chat_req.get("system", "") or (msgs[0].get("content", "") if msgs and msgs[0].get("role") == "system" else ""))
